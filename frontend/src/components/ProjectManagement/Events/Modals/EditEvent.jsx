...
const EditEvent = ({
    ...
    useEffect(() => {
        if (selectedEvent) {
            ...
            setSelectedClient(selectedEvent.client || null);
            setShareWithOptions(selectedEvent.shareWithOptions || 'only_me');
            setSelectedSpecificMembers(selectedEvent.specificMembers || []);
            setRepeat(selectedEvent.repeat || false);
            setRepeatEvery(selectedEvent.repeatEvery || '');
            setSelectedEveryParam(selectedEvent.selectedEveryParam || '');
            setCycles(selectedEvent.cycles || '');
            setColor(selectedEvent.color || 'gray');
        }
    }, [selectedEvent, setTitle, setDescription, setStartDate, setEndDate, setStartTime, setEndTime, setLocation, setLabel, setSelectedClient, setShareWithOptions, setSelectedSpecificMembers, setRepeat, setRepeatEvery, setSelectedEveryParam, setCycles, setColor]);
    ...
    return (
        <div>
           <Dialog open={isOpen} className="custom-dialog">
            <form id="event-form" onSubmit={handleEditEventSubmit}>
                  ...
                  <div className="flex mb-4">
                    ...
                    <div className="w-9/12">
                      <Select
                        ...
                        onChange={(option) => handleClientChange(option ? option.value : null)}
                  ...
                  <div className="flex mb-4">
                    ...
                    />
                      ...
                      {shareWithOptions === 'specific_members_teams' && (
                        <Select
                          id="specificMembers"
                          value={specificMembersOptions.filter(option =>
                            selectedSpecificMembers.includes(option.value)
                          )}
                          onChange={(options) => handleSpecificMembersChange(options.map(option => option.value))}
                      ...
                      <div className="flex mb-4">
                        ...
                        <div className="flex w-9/12 gap-5">
                          ...
                          <Select
                            ...
                            value={everyParamOptions.find(option => option.value === selectedEveryParam)}
                            onChange={(option) => setSelectedEveryParam(option ? option.value : '')}
                            className="react-select-container"
                            classNamePrefix="react-select"
                            isClearable
                          />
